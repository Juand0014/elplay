...
-- NOTE: Helper functions live in `public` schema (not `auth`) —
--       Supabase SQL Editor does not allow writing to auth schema.
-- ============================================================
...
DO $$ BEGIN CREATE TYPE tipo_partido       AS ENUM ('liga', 'torneo', 'interno');    EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE estado_partido     AS ENUM ('pending', 'live', 'done', 'ko'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE media_entrada      AS ENUM ('top', 'bottom');                 EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE tipo_jugada        AS ENUM ('1B','2B','3B','HR','BB','HBP','K','OUT','E','DP'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE posicion_campo     AS ENUM ('P','C','1B','2B','3B','SS','LF','CF','RF','DH'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE formato_torneo     AS ENUM ('SE', 'DE', 'GE');               EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE estado_torneo      AS ENUM ('draft','open','active','done');  EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE tipo_inscripcion   AS ENUM ('self', 'admin');                EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE visibilidad_apunte AS ENUM ('privado','equipo','publico');    EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE categoria_apunte   AS ENUM ('partido','tactica','jugador','resumen','general'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE rol_usuario        AS ENUM ('comisionado','dueno_equipo','scorer','miembro','publico'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS partidos (
  ...
  CONSTRAINT liga_requerida_para_tipo_liga    CHECK (tipo <> 'liga'   OR liga_id    IS NOT NULL),
  CONSTRAINT torneo_requerido_para_tipo_torneo CHECK (tipo <> 'torneo' OR torneo_id IS NOT NULL)
);
...
CREATE TABLE IF NOT EXISTS roles_usuario (
  ...
  CONSTRAINT rol_debe_tener_contexto CHECK (liga_id IS NOT NULL OR equipo_id IS NOT NULL)
);
...
DO $$ BEGIN CREATE TRIGGER set_updated_at_ligas     BEFORE UPDATE ON ligas     FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TRIGGER set_updated_at_equipos   BEFORE UPDATE ON equipos   FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TRIGGER set_updated_at_jugadores BEFORE UPDATE ON jugadores FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TRIGGER set_updated_at_torneos   BEFORE UPDATE ON torneos   FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TRIGGER set_updated_at_partidos  BEFORE UPDATE ON partidos  FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TRIGGER set_updated_at_apuntes   BEFORE UPDATE ON apuntes   FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ============================================================
-- RLS HELPER FUNCTIONS  (public schema — auth schema is off-limits)
-- These are SECURITY DEFINER so they run with the owner's privileges.
-- ============================================================

CREATE OR REPLACE FUNCTION public.es_comisionado(p_liga_id UUID)
...
CREATE OR REPLACE FUNCTION public.es_dueno_equipo(p_equipo_id UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM roles_usuario
...
CREATE OR REPLACE FUNCTION public.es_scorer(p_equipo_id UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM roles_usuario
...
CREATE OR REPLACE FUNCTION public.es_miembro_equipo(p_equipo_id UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM roles_usuario
    WHERE usuario_id = auth.uid() AND equipo_id = p_equipo_id
...
CREATE OR REPLACE FUNCTION public.puede_anotar_partido(p_partido_id UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    ...
    WHERE p.id = p_partido_id
      AND (
        ...
        OR public.es_dueno_equipo(p.equipo_local_id)
        OR public.es_dueno_equipo(p.equipo_visitante_id)
        OR (p.liga_id IS NOT NULL AND public.es_comisionado(p.liga_id))
      )
  );
$$;
...
CREATE POLICY "equipos_insert"             ON equipos FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND (public.es_comisionado(liga_id) OR dueno_id = auth.uid()));
CREATE POLICY "equipos_update"             ON equipos FOR UPDATE USING (public.es_comisionado(liga_id) OR dueno_id = auth.uid()) WITH CHECK (public.es_comisionado(liga_id) OR dueno_id = auth.uid());
CREATE POLICY "equipos_delete_comisionado" ON equipos FOR DELETE USING (public.es_comisionado(liga_id));
...
CREATE POLICY "jugadores_select_equipo"  ON jugadores FOR SELECT USING (public.es_dueno_equipo(equipo_id) OR public.es_miembro_equipo(equipo_id) OR usuario_id = auth.uid());
CREATE POLICY "jugadores_insert"         ON jugadores FOR INSERT WITH CHECK (public.es_dueno_equipo(equipo_id) OR EXISTS (SELECT 1 FROM equipos e WHERE e.id = equipo_id AND public.es_comisionado(e.liga_id)));
CREATE POLICY "jugadores_update"         ON jugadores FOR UPDATE USING (public.es_dueno_equipo(equipo_id) OR EXISTS (SELECT 1 FROM equipos e WHERE e.id = equipo_id AND public.es_comisionado(e.liga_id)));
...
CREATE POLICY "torneos_select" ON torneos FOR SELECT USING (estado <> 'draft' OR organizador_id = auth.uid() OR (liga_id IS NOT NULL AND public.es_comisionado(liga_id)));
CREATE POLICY "torneos_insert" ON torneos FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND organizador_id = auth.uid() AND (liga_id IS NULL OR public.es_comisionado(liga_id)));
CREATE POLICY "torneos_update" ON torneos FOR UPDATE USING (organizador_id = auth.uid() OR (liga_id IS NOT NULL AND public.es_comisionado(liga_id)));
...
CREATE POLICY "inscripciones_insert" ON inscripciones FOR INSERT WITH CHECK (public.es_dueno_equipo(equipo_id) OR EXISTS (SELECT 1 FROM torneos t WHERE t.id = torneo_id AND t.organizador_id = auth.uid()));
CREATE POLICY "inscripciones_delete" ON inscripciones FOR DELETE USING (public.es_dueno_equipo(equipo_id) OR EXISTS (SELECT 1 FROM torneos t WHERE t.id = torneo_id AND t.organizador_id = auth.uid()));
...
CREATE POLICY "ko_rules_insert"         ON knockout_rules FOR INSERT WITH CHECK ((liga_id IS NOT NULL AND public.es_comisionado(liga_id)) OR (torneo_id IS NOT NULL AND EXISTS (SELECT 1 FROM torneos t WHERE t.id = torneo_id AND t.organizador_id = auth.uid())) OR (partido_id IS NOT NULL AND public.puede_anotar_partido(partido_id)));
CREATE POLICY "ko_rules_update"         ON knockout_rules FOR UPDATE USING ((liga_id IS NOT NULL AND public.es_comisionado(liga_id)) OR (torneo_id IS NOT NULL AND EXISTS (SELECT 1 FROM torneos t WHERE t.id = torneo_id AND t.organizador_id = auth.uid())) OR (partido_id IS NOT NULL AND public.puede_anotar_partido(partido_id)));
...
CREATE POLICY "partidos_select_interno" ON partidos FOR SELECT USING (tipo = 'interno' AND (public.es_miembro_equipo(equipo_local_id) OR public.es_dueno_equipo(equipo_local_id) OR public.es_scorer(equipo_local_id) OR (liga_id IS NOT NULL AND public.es_comisionado(liga_id))));
CREATE POLICY "partidos_insert"         ON partidos FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND ((tipo = 'liga' AND liga_id IS NOT NULL AND public.es_comisionado(liga_id)) OR (tipo = 'torneo' AND torneo_id IS NOT NULL AND EXISTS (SELECT 1 FROM torneos t WHERE t.id = torneo_id AND t.organizador_id = auth.uid())) OR (tipo = 'torneo' AND liga_id IS NOT NULL AND public.es_comisionado(liga_id)) OR (tipo = 'interno' AND public.es_dueno_equipo(equipo_local_id))));
CREATE POLICY "partidos_update"         ON partidos FOR UPDATE USING (public.puede_anotar_partido(id));
...
CREATE POLICY "jugadas_select_interno" ON jugadas FOR SELECT USING (EXISTS (SELECT 1 FROM partidos p WHERE p.id = partido_id AND p.tipo = 'interno' AND (public.es_miembro_equipo(p.equipo_local_id) OR public.es_dueno_equipo(p.equipo_local_id) OR public.es_scorer(p.equipo_local_id))));
CREATE POLICY "jugadas_insert"         ON jugadas FOR INSERT WITH CHECK (public.puede_anotar_partido(partido_id) AND anotado_por = auth.uid());
CREATE POLICY "jugadas_delete"         ON jugadas FOR DELETE USING (public.puede_anotar_partido(partido_id));
...
CREATE POLICY "juego_grupos_select" ON juego_grupos FOR SELECT USING (EXISTS (SELECT 1 FROM partidos p WHERE p.id = partido_id AND (public.es_miembro_equipo(p.equipo_local_id) OR public.es_dueno_equipo(p.equipo_local_id) OR public.es_scorer(p.equipo_local_id))));
CREATE POLICY "juego_grupos_insert" ON juego_grupos FOR INSERT WITH CHECK (public.puede_anotar_partido(partido_id));
CREATE POLICY "juego_grupos_update" ON juego_grupos FOR UPDATE USING (public.puede_anotar_partido(partido_id));
CREATE POLICY "juego_grupos_delete" ON juego_grupos FOR DELETE USING (public.puede_anotar_partido(partido_id));
...
CREATE POLICY "grupo_jugadores_select" ON grupo_jugadores FOR SELECT USING (EXISTS (SELECT 1 FROM juego_grupos jg JOIN partidos p ON p.id = jg.partido_id WHERE jg.id = grupo_id AND (public.es_miembro_equipo(p.equipo_local_id) OR public.es_dueno_equipo(p.equipo_local_id))));
CREATE POLICY "grupo_jugadores_insert" ON grupo_jugadores FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM juego_grupos jg WHERE jg.id = grupo_id AND public.puede_anotar_partido(jg.partido_id)));
CREATE POLICY "grupo_jugadores_delete" ON grupo_jugadores FOR DELETE USING (EXISTS (SELECT 1 FROM juego_grupos jg WHERE jg.id = grupo_id AND public.puede_anotar_partido(jg.partido_id)));
...
CREATE POLICY "apuntes_select_equipo"  ON apuntes FOR SELECT USING (visibilidad = 'equipo' AND equipo_id IS NOT NULL AND (public.es_miembro_equipo(equipo_id) OR public.es_dueno_equipo(equipo_id) OR public.es_scorer(equipo_id)));
...
CREATE POLICY "apuntes_delete"         ON apuntes FOR DELETE USING (usuario_id = auth.uid() OR (equipo_id IS NOT NULL AND EXISTS (SELECT 1 FROM equipos e WHERE e.id = equipo_id AND public.es_comisionado(e.liga_id))));
...
CREATE POLICY "roles_select_comisionado" ON roles_usuario FOR SELECT USING (liga_id IS NOT NULL AND public.es_comisionado(liga_id));
CREATE POLICY "roles_select_dueno"       ON roles_usuario FOR SELECT USING (equipo_id IS NOT NULL AND public.es_dueno_equipo(equipo_id));
CREATE POLICY "roles_insert"             ON roles_usuario FOR INSERT WITH CHECK ((liga_id IS NOT NULL AND public.es_comisionado(liga_id)) OR (equipo_id IS NOT NULL AND public.es_dueno_equipo(equipo_id)) OR (rol = 'miembro' AND equipo_id IS NOT NULL AND usuario_id = auth.uid() AND EXISTS (SELECT 1 FROM jugadores j WHERE j.equipo_id = roles_usuario.equipo_id AND j.usuario_id = auth.uid())));
CREATE POLICY "roles_delete"             ON roles_usuario FOR DELETE USING (usuario_id = auth.uid() OR (liga_id IS NOT NULL AND public.es_comisionado(liga_id)) OR (equipo_id IS NOT NULL AND public.es_dueno_equipo(equipo_id)));
